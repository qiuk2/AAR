...
class AAR(nn.Module):
    def __init__(
        ...
        self.Cvae, self.V = vae_local.quantizer.vq.dim, vae_local.quantizer.vq.layers[0].codebook_size # vae_local.quantizer.bins
        ...
        quant: ResidualVectorQuantizer = vae_local.quantizer
        self.vae_proxy: Tuple[EncodecModel] = (vae_local,)
        self.vae_quant_proxy: Tuple[ResidualVectorQuantizer] = (quant,)
        self.word_embed = nn.Linear(self.Cvae, self.C)
        
        # 2. class embedding
        self.class_emb = MLP(in_features=input_dim, hidden_features=self.C, out_features=self.D)
        self.register_buffer("uncond_embedding", nn.Parameter(torch.randn(1, 512) / 512 ** 0.5))
    ...
    def autoregressive_infer_cfg(
        ...
        for si, pn in enumerate(self.patch_nums):   # si: i-th segment
            ...
            if not more_smooth:
                h_BChw = self.vae_quant_proxy[0].embedding(idx_Bl, si)   # B, l, Cvae
            else:
                gum_t = max(0.27 * (1 - ratio * 0.95), 0.005)   # refer to mask-git
                h_BChw = gumbel_softmax_with_rng(logits_BlV.mul(1 + ratio), tau=gum_t, hard=False, dim=-1, rng=rng) @ self.vae_quant_proxy[0].embedding.weight.unsqueeze(0)
            
            h_BChw = h_BChw.reshape(B, self.Cvae, pn)
            f_hat, next_token_map = self.vae_quant_proxy[0].get_next_autoregressive_input(si, f_hat, h_BChw)
        ...
        return self.vae_proxy[0].fhat_to_audio(f_hat)   # de-normalize, from [-1, 1] to [0, 1]
