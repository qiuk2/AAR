...
class DiscriminatorSTFT(nn.Module):
    ...
    def __init__(self, filters: int, in_channels: int = 1, out_channels: int = 1,
        ...
        self.spec_transform = tl.STFT(n_fft=self.n_fft, hop_length=self.hop_length, 
    ...
    def forward(self, x: torch.Tensor):
        ...
        """
        fmap = []
        real, imag = self.spec_transform(x.squeeze(1,2))  # [B, 2, Frames, Freq, 2]
        if self.normalized:
            norm_factor = torch.sqrt(torch.tensor(self.n_fft / 2, device= x.device))
            real, imag = real / norm_factor, imag / norm_factor
